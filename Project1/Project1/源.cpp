...
int main()
{
	...
	if (str.find("<+>") != string::npos) {
		auto p = str.find("<+>");
		Number a(str.substr(0, p)), b(str.substr(p + 3));
		if (!a.check() || !b.check()) 
			cout << "error";
		else
			expression.add(a, b).print();
	}
		...
		if (!a.check() || !b.check())
			...
			expression.times(a, b).print();
	}
	else if (str.find("</>") != string::npos) {
		auto p = str.find("</>");
		Number a(str.substr(0, p)), b(str.substr(p + 3));
		if (!a.check() || !b.check() || a.is_rnum || b.is_rnum)
			cout << "error";
		else
